...
options.registry.js_get_static_selectFilter =  options.Class.extend({
	start: function() {
	      ...
	      var self      = this;
	      rpc.query({
	    	    model: 'website.multi.filter.ept',
	    	    method: 'search_read',
	    	    args: [[['website_published','=',true]],['name','filter_ids','id']],
	    ...
	    createwebsitefiltersList: function(filters)
	    {
	    	...
	    	if (self.$target.attr("data-filter_static_by_filter_id")) {
	    			...
	    			console.log(id)
	    			console.log(ul.find("li[data-filter_by_filter_id=" + id  + "]"))
	    ...
	    filter_static_by_filter_id:function(previewMode, value){
	        ...
	        this.$target.attr("data-filter_static_by_filter_id",value)
