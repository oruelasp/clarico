...
class claricoBrand(claricoShop):
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        if response.qcontext.get('products', False):
            products = response.qcontext.get('products')
            brand_ids = []
            for prod in products:
                if prod.brand_ept_id and prod.brand_ept_id.is_website_publish == True and prod.brand_ept_id not in brand_ids:
                    brand_ids.append(prod.brand_ept_id)
            if len(brand_ids) > 0:
                response.qcontext['brand_list'] = brand_ids
        return response